...
> Note for EU residence users:
> 
> Add the following code in the `setup.js` file for both `createClient` and `spaceImport`
> ```js
>   baseURL: 'https://api.eu.contentful.com'
> ```
> Also update the hostname in `lib/helpers.js` and `next.config.js`.
